...
def split_data(features, labels):
  ...
  return normalize(np.array(train_features)), np.array(train_labels), normalize(np.array(test_features)), np.array(test_labels)
...
def plot_incorrect_images(X_test, y_test, y_pred, model_name):
    ...
    wrong_x, wrong_y, wrong_pred = X_test[wrong_predictions], y_test[wrong_predictions], np.array(y_pred)[wrong_predictions]
...
def fit_and_predict_model(model, model_name, X_train, y_train, X_test, y_test, data_augment):
    start = time.time()
    if data_augment:
    ...
    print(f"The model {model_name} {augmentation_sent} took {time.time() - start} seconds to train and results in an accuracy of {accuracy}")
...
def fit_MLP_model(X_train, y_train, X_test, y_test, data_augment):
    ...
    history = model.fit(
        normalize(X_train), tf.keras.utils.to_categorical(y_train),
    ...
    print(f"The model MLP {augmentation_sent} took {time.time() - start} seconds to train and results in an accuracy of {accuracy}")
...
def fit_CNN_model(X_train, y_train, X_test, y_test, data_augment):
    ...
    model.add(tf.keras.layers.Conv2D(16, (3,3), activation='relu', input_shape=(16, 15, 1)))
    ...
    history = model.fit(
        X_train.reshape(-1, 16, 15, 1), tf.keras.utils.to_categorical(y_train),
        validation_split=0.1,
        epochs=50,
        callbacks=callbacks
    )
    ...
    print(f"The model CNN {augmentation_sent} took {time.time() - start} seconds to train and results in an accuracy of {accuracy}")